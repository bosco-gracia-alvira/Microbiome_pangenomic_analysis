...
metadata_isolates_path <- "/Users/bgracia/PopGen Dropbox/Martin McFly/Bosco/PhD_Dropbox/Microbiome_pangenomic_analysis/data/Lactiplantibacillus_plantarum/Anvio_pangen/Anvio_misc.tsv"
...
freq <- fread(paste0(data_path,"/Lactiplantibacillus_plantarum.freq"),sep="\t",header=T, colClasses = "character")
...
ggplot(SFS, aes(x = RO)) +
  ...
  labs(title = "Site Frequency Spectrum (Folded)", x = "Allele Frequency", y = "Count") +
  theme_minimal() +
  theme(
...
ggplot(SFS, aes(x = RO)) +
  ...
  facet_wrap(~ Sample, scales = "free_y", labeller = labeller(Sample = function(x) {
    paste0(x," (",SFS %>% filter(Sample == x) %>% pull(allele_count) %>% unique(),")")
  }))
